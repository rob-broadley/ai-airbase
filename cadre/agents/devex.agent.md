...
- Never create **generated** project-local environments or dependency directories inside `/workspace` (e.g. `.venv`, `node_modules`, `.gradle`, `target/`). The workspace is a bind mount of the host project — writing generated artefacts into it can corrupt the host's own tooling. If a project-local environment is needed, create it under `$HOME` instead (e.g. `uv venv ~/.venvs/<project>`). Note: source-controlled directories like Go's `vendor/` that are committed to the repo are fine — do not remove or recreate them..
